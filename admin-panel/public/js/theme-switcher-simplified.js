/**
 * Vision UI Dashboard - Theme Switcher (Simplified version)
...
function loadSavedTheme() {
  try {
    ...
    if (savedSettings) {
      ...
      // اعمال تنظیمات دیگر
      ['glassmorphism', 'animations', 'hover-effects'].forEach(function(setting) {
...
function resetThemeSettings() {
  ...
  // به‌روزرسانی توگل‌ها
  const settingToggles = document.querySelectorAll('.theme-settings-toggle');
  settingToggles.forEach(function(toggle) {
    toggle.classList.add('active');
  });
