...
const werewolfRoles: Record<string, WerewolfRole> = {
  villager: {
    name: 'روستایی',
    description: 'با سایر روستاییان همکاری کن تا گرگینه‌ها رو پیدا کنی.',
    emoji: '👨‍🌾',
    team: 'village'
  },
  werewolf: {
    name: 'گرگینه',
    description: 'هر شب با گرگینه‌های دیگر همکاری کن و یکی از روستاییان رو حذف کن.',
    emoji: '🐺',
    team: 'werewolf',
    nightAction: true
  },
  seer: {
    name: 'پیشگو',
    description: 'هر شب می‌تونی هویت واقعی یک نفر رو ببینی.',
    emoji: '🔮',
    team: 'village',
    nightAction: true
  },
  doctor: {
    name: 'پزشک',
    description: 'هر شب می‌تونی یک نفر رو از حمله گرگینه‌ها محافظت کنی.',
    emoji: '💉',
    team: 'village',
    nightAction: true
  },
  hunter: {
    name: 'شکارچی',
    description: 'اگر کشته بشی، می‌تونی یک نفر رو با خودت ببری.',
    emoji: '🏹',
    team: 'village'
  },
  bodyguard: {
    name: 'محافظ',
    description: 'هر شب می‌تونی از یک بازیکن محافظت کنی. اگر گرگینه‌ها به اون حمله کنند، تو به جاش می‌میری.',
    emoji: '🛡️',
    team: 'village',
    nightAction: true
  }
...
export async function createWerewolfGame(interaction: ButtonInteraction) {
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription('تو یه دهکده پر از گرگینه‌ها گیر افتادی! باید با شهروندا همکاری کنی تا گرگینه‌ها رو پیدا کنی، یا اگه گرگینه هستی، شب‌ها شکار کنی! 🌙')
      .setColor(Colors.DarkBlue)
      .addFields(
        { name: '👥 تعداد بازیکنان', value: `1/${gameSession.settings.maxPlayers}`, inline: true },
        { name: '⏱️ زمان هر روز', value: '5 دقیقه', inline: true },
        { name: '🌃 زمان هر شب', value: '3 دقیقه', inline: true },
        { name: '👤 حداقل بازیکنان', value: `${gameSession.settings.minPlayers} نفر`, inline: true },
...
export async function joinWerewolfGame(interaction: ButtonInteraction) {
  try {
    ...
    const embed = EmbedBuilder.from(message.embeds[0])
      .setFields(
        { name: '👥 تعداد بازیکنان', value: `${gameData.players.length}/${gameSession.settings.maxPlayers}`, inline: true },
        { name: '⏱️ زمان هر روز', value: '5 دقیقه', inline: true },
        { name: '🌃 زمان هر شب', value: '3 دقیقه', inline: true },
        { name: '👤 حداقل بازیکنان', value: `${gameSession.settings.minPlayers} نفر`, inline: true },
        { name: '💰 جایزه بازی', value: 'برنده 500 کوین 🤑', inline: true },
        { name: '🏠 میزبان', value: `${(await interaction.guild?.members.fetch(gameData.hostId))?.user.username || 'نامشخص'}`, inline: true }
      );
