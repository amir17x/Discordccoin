...
async function sendTip(client: Client, settings: TipChannelSettings) {
  try {
    ...
    // تعیین اینکه آیا از هوش مصنوعی استفاده شود (افزایش احتمال به 90% - تاکید بیشتر روی نکات هوشمند CCOIN AI)
    const useAI = Math.random() < 0.9;
    ...
    const embed = new EmbedBuilder()
      ...
      .setTitle(useAI ? '🧠 نکته هوشمند CCOIN AI' : '💡 نکته روز')
