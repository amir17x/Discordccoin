  ...
  try {
    ...
    if (useAI) {
      try {
        ...
        // استفاده از کتابخانه محلی در صورت خطا - با پیام‌های کوتاه و خوانا
        const backupMessages = [
          "🧠 استراحت کوتاه هوش مصنوعی",
          "🔌 شارژ مجدد مغز الکترونیکی",
          "💤 چرت کوتاه هوش مصنوعی",
          "🌙 خواب کوتاه مدت AI",
          "🧪 در حال آپدیت سیستم",
          "📡 تنظیم آنتن‌های هوشمند",
          "🔄 بازیابی حافظه مصنوعی",
          "⚡ شارژ مجدد باتری‌های AI"
        ];
        statusMessage = backupMessages[Math.floor(Math.random() * backupMessages.length)];
      }
    } else {
