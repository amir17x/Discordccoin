...
/**
 * تابع کمکی برای نمایش نوار پیشرفت سلامتی
 * @param current میزان فعلی سلامتی
 * @param max حداکثر میزان سلامتی
 * @returns رشته نمایش نوار پیشرفت
 */
function createHealthBar(current: number, max: number = PLAYER_HEALTH): string {
  // مطمئن می‌شویم که مقدار سلامتی منفی نباشد
  const safeValue = Math.max(0, current);
  // محاسبه درصد سلامتی
  const percent = Math.floor((safeValue / max) * 100);
  // محاسبه تعداد بلوک‌های پر و خالی
  const filledBlocks = Math.floor((percent / 100) * 10);
      ...
      case 'match':
        ...
        const gameEmbed = new EmbedBuilder()
          ...
          .addFields(
            ...
            { name: `❤️ **\`<@${interaction.user.id}>\`**`, value: createHealthBar(PLAYER_HEALTH), inline: true },
            { name: `❤️ **\`<@${targetId}>\`**`, value: createHealthBar(PLAYER_HEALTH), inline: true },
  ...
  try {
    ...
    if (channel && 'send' in channel) {
      ...
      const gameEmbed = new EmbedBuilder()
        ...
        .addFields(
          { name: '📊 وضعیت', value: 'انتخاب اسلحه توسط بازیکنان', inline: false },
          { name: `❤️ **\`<@${player1Id}>\`**`, value: `${'█'.repeat(10)} 100%`, inline: true },
          { name: `❤️ **\`<@${player2Id}>\`**`, value: `${'█'.repeat(10)} 100%`, inline: true },
          { name: '🔄 دور', value: '1', inline: false }
        )
        .setFooter({ text: 'هر بازیکن باید یک اسلحه انتخاب کند!' })
        .setTimestamp();
